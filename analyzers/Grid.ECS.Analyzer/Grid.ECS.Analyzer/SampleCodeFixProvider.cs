...
public class SampleCodeFixProvider : CodeFixProvider
{
	...
	public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context) {
		...
		context.RegisterCodeFix(
			CodeAction.Create(
				title: string.Format(Resources.AB0001CodeFixTitle, SampleSyntaxAnalyzer.CompanyName,
					CommonName),
				createChangedSolution: c =>
					SanitizeCompanyNameAsync(context.Document, declaration, c),
	...
	private async Task<Solution> SanitizeCompanyNameAsync(Document document,
		...
		var newName =
			classDeclarationSyntax.Identifier.Text.Replace(SampleSyntaxAnalyzer.CompanyName,
				CommonName);
		...
		var newSolution = await Renamer
			.RenameSymbolAsync(document.Project.Solution, typeSymbol, new SymbolRenameOptions(),
				newName,
				cancellationToken)
			.ConfigureAwait(false);
