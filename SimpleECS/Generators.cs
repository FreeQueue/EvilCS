...
{
    private static string Pattern(string value, int count, bool comma_sep = true) {
        string result = "";
    ...
    public static void CreateEntityFunctions(int count, string output_path) {
        using (var writer = new StreamWriter(output_path)) {
            ...
            for (int i = 1; i < count + 1; ++i) {
                writer.WriteLine(
                    $"      public Entity CreateEntity<{Pattern("C#", i)}>({Pattern("C# c#", i)})");
                writer.WriteLine("      {");
                writer.WriteLine("          if (this.TryGetWorldInfo(out var data))");
                writer.WriteLine("          {");
                writer.WriteLine(
                    $"              data.buffer_signature.Clear(){Pattern(".Add<C#>()", i, false)};");
                writer.WriteLine(
    ...
    public static void CreateQueryFunctions(int world_data_count, int component_count,
        string output_path) {
        using (var writer = new StreamWriter(output_path)) {
            writer.WriteLine($"namespace {nameof(SimpleECS)}");
            writer.WriteLine("{");
            ...
            for (int c = 1; c < component_count + 1; ++c) // components
            {
                ...
                string arch_get = Pattern("&& archetype.TryGetArray(out C#[] c#)", c, false);

                writer.WriteLine(
                    $"       public delegate void c{c}_query<{c_val}>({Pattern("ref C# c#", c)});");
                WriteDocumentation();
                writer.WriteLine(
                    $"       public void Foreach<{c_val}>(in c{c}_query<{c_val}> action)");
                writer.WriteLine("       {");
            ...
            for (int c = 1; c < component_count + 1; ++c) // entity and components
            {
                ...
                writer.WriteLine(
                    $"       public delegate void ec{c}_query<{c_val}>(Entity entity, {Pattern("ref C# c#", c)});");
                WriteDocumentation();
                writer.WriteLine(
                    $"       public void Foreach<{c_val}>(in ec{c}_query<{c_val}> action)");
                writer.WriteLine("       {");
                writer.WriteLine("           if (Update(out var world_info))");
                writer.WriteLine("           {");
                writer.WriteLine("               world_info.StructureEvents.EnqueueEvents++;");
                ...
                writer.WriteLine("                       for (int e = 0; e < count; ++e)");
                writer.WriteLine(
                    $"                       action(entities[e], {Pattern("ref c#[e]", c)});");
                writer.WriteLine("                   }");
                writer.WriteLine("               }");
                writer.WriteLine("               world_info.StructureEvents.EnqueueEvents--;");
                writer.WriteLine("           }");
            ...
            for (int w = 1; w < world_data_count + 1; ++w) {
                ...
                writer.WriteLine(
                    $"       public delegate void w{w}c{c}_query<{w_val},{c_val}>({Pattern("in W# w#", w)}, {Pattern("ref C# c#", c)});");
                WriteDocumentation();
                writer.WriteLine(
                    $"       public void Foreach<{w_val},{c_val}>(in w{w}c{c}_query<{w_val},{c_val}> action)");
                writer.WriteLine("       {");
                writer.WriteLine("           if (Update(out var world_info))");
                writer.WriteLine("           {");
                writer.WriteLine(
                    $"{Pattern("               ref W# w# = ref world.GetData<W#>();\n", w, false)}");
                writer.WriteLine("               world_info.StructureEvents.EnqueueEvents++;");
                writer.WriteLine(
                    "               for (int archetype_index = 0; archetype_index < archetype_count; ++archetype_index)");
                writer.WriteLine("               {");
                ...
                writer.WriteLine("                       for (int e = 0; e < count; ++e)");
                writer.WriteLine(
                    $"                       action({Pattern("in w#", w)}, {Pattern("ref c#[e]", c)});");
                writer.WriteLine("                   }");
                writer.WriteLine("               }");
                writer.WriteLine("               world_info.StructureEvents.EnqueueEvents--;");
                writer.WriteLine("           }");
            ...
            for (int w = 1; w < world_data_count + 1; ++w) {
                ...
                writer.WriteLine(
                    $"       public delegate void w{w}ec{c}_query<{w_val},{c_val}>({Pattern("in W# w#", w)}, Entity entity, {Pattern("ref C# c#", c)});");
                WriteDocumentation();
                writer.WriteLine(
                    $"       public void Foreach<{w_val},{c_val}>(in w{w}ec{c}_query<{w_val},{c_val}> action)");
                writer.WriteLine("       {");
                writer.WriteLine("           if (Update(out var world_info))");
                writer.WriteLine("           {");
                writer.WriteLine(
                ...
                writer.WriteLine("                       for (int e = 0; e < count; ++e)");
                writer.WriteLine(
                    $"                       action({Pattern("in w#", w)}, entities[e], {Pattern("ref c#[e]", c)});");
                writer.WriteLine("                   }");
                writer.WriteLine("               }");
                writer.WriteLine("               world_info.StructureEvents.EnqueueEvents--;");
                writer.WriteLine("           }");
            ...
            void WriteDocumentation() {
                ...
                writer.WriteLine(
                    $"        /// query can add up to {world_data_count} world data and {component_count} components");
                writer.WriteLine("        /// </summary>");
            }
        }
    }
